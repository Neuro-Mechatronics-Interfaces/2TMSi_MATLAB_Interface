function [net, meta, input_data] = train_envelope_regressor_from_poly5(poly5_file, options)
%TRAIN_ENVELOPE_REGRESSOR_FROM_POLY5  Trains envelope convolutional regressor network directly from poly5 file.
arguments
...
[net, meta, input_data] = train_envelope_regressor(data.samples(options.Channels,:), ...
    ... labels, ...
    Y, ...
...
if options.AutoSave
    [p,f,~] = fileparts(poly5_file);
    net_file_out = fullfile(p, sprintf('%s_envelope-regressor-parameters.mat', f));
    save(net_file_out, 'meta', 'options', 'S', 'labels', 'instructionList', 'Y', '-v7.3');
    net_file_out = fullfile(p, sprintf('%s_envelope-regressor.mat', f));
    Net = net;
