classdef Library < HiddenHandle
    ...
    methods
        ...
        function devices = getDevices(obj, ds_interface, dr_interface, numOfRetries)
            ...
            for k = 1:numel(ds_interface)
                for j = 1:numel(dr_interface)
                    try
                        device_list = DeviceLib.getDeviceList(...
                            TMSiUtils.toInterfaceTypeNumber(ds_interface{k}), ...
                            TMSiUtils.toInterfaceTypeNumber(dr_interface{j}), numOfRetries);
                    catch
                    ...
                    for i=1:numel(device_list)
                        devices = vertcat(devices, Device(obj, int64(device_list(i).TMSiDeviceID), dr_interface{j})); %#ok<AGROW>
                    end
                end
            end
            if isempty(devices)
        ...
        function device = getDevice(obj, device_id, dr_interface)
            ...
            device = Device(obj, int64(device_id), dr_interface);
        end
    ...
    methods (Static, Access = public)
        ...
        function loadLibrary()
            if Library.isLoaded()
                ...
                loadlibrary('TMSiSagaDeviceLib.dll', @TMSiSagaDeviceLib64);
            end
        end
