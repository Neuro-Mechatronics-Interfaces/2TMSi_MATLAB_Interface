classdef DeviceLib < HiddenHandle
    ...
    methods (Static = true)
        ...
        function device_list = getDeviceList(ds_interface_type, dr_interface_type, maxNumOfRetries, max_num_devices)
            ...
            device_list_buffer = TMSiUtils.toLibraryStructure('TMSiDevList', repmat(struct(), 1, max_num_devices));
            ...
            % Call dll function
            while nrOfRetries > 0
                status = calllib(DeviceLib.alias(), 'TMSiGetDeviceList', device_list_buffer, ...
            ...
            if status ~= 0
                throw(MException('DeviceLib:getDeviceList', 'failed because of %s', ...
                    TMSiUtils.getErrorString(status)));
            end
        ...
        function handle = openDevice(device_id, dr_interface_type)
            ...
            % Call dll function
            status = calllib(DeviceLib.alias(), 'TMSiOpenDevice', handle, device_id, ...
                dr_interface_type);
        ...
        function handle = closeDevice(handle)
            ...
            % Call dll function
            status = calllib(DeviceLib.alias(), 'TMSiCloseDevice', handle);
            if status ~= 0
        ...
        function device_report = getDeviceStatus(handle)
            ...
            device_report = TMSiUtils.toLibraryStructure('TMSiDevStatReport', struct());

            % Call dll function
            status = calllib(DeviceLib.alias(), 'TMSiGetDeviceStatus', handle, device_report);
            if status ~= 0
        ...
        function [device_report, battery_report_list, ...
            ...
            device_report = TMSiUtils.toLibraryStructure('TMSiDevFullStatReport', struct());
            battery_reports_ptr = TMSiUtils.toLibraryStructure('TMSiDevBatReport', repmat(struct(), 1, num_batteries));
            time_report = TMSiUtils.toLibraryStructure('TMSiTime', struct());
            storage_report = TMSiUtils.toLibraryStructure('TMSiDevStorageReport', struct());

            % Call dll function
            status = calllib(DeviceLib.alias(), 'TMSiGetFullDeviceStatus', ...
        ...
        function [recorder_configuration, channel_list] = getDeviceConfig(handle, num_channels)
            ...
            recorder_configuration = TMSiUtils.toLibraryStructure('TMSiDevGetConfig', struct());
            channel_list_ptr = TMSiUtils.toLibraryStructure('TMSIDevChDesc', repmat(struct(), 1, num_channels));
            
            % Call dll function
            status = calllib(DeviceLib.alias(), 'TMSiGetDeviceConfig', ...
            ...
            recorder_configuration = TMSiUtils.toMatlabStructure('TMSiDevGetConfig', recorder_configuration, 1);
            channel_list = TMSiUtils.toMatlabStructure('TMSIDevChDesc', channel_list_ptr, num_channels);
        ...
        function setDeviceConfig(handle, recorder_config, channel_configs)
            ...
            rc = TMSiUtils.toLibraryStructure('TMSiDevSetConfig', recorder_config);
            cc = TMSiUtils.toLibraryStructure('TMSIDevSetChCfg', channel_configs);

            % Call dll function
            status = calllib(DeviceLib.alias(), 'TMSiSetDeviceConfig', handle, ...
        ...
        function setDeviceSampling(handle, deviceSamplingMode)
            ...
            deviceSamplingMode = TMSiUtils.toLibraryStructure('TMSiDevSampleReq', deviceSamplingMode);

            % Call dll function
            status = calllib(DeviceLib.alias(), 'TMSiSetDeviceSampling', handle, ...
                deviceSamplingMode);
            if status ~= 0
        ...
        function setDeviceImpedance(handle, deviceImpedanceMode)
            ...
            deviceImpedanceMode = TMSiUtils.toLibraryStructure('TMSiDevImpReq', deviceImpedanceMode);
            
            % Call dll function
            status = calllib(DeviceLib.alias(), 'TMSiSetDeviceImpedance', handle, ...
                deviceImpedanceMode);
            if status ~= 0
        ...
        function [data, num_sets, data_type] = getDeviceData(handle, buffer, max_size)
            ...
            % Call dll function
            status = calllib(DeviceLib.alias(), 'TMSiGetDeviceData', handle, ...
        ...
        function sensors = getDeviceSensors(handle, num_sensors)
            ...
            channel_list_ptr = TMSiUtils.toLibraryStructure('TMSiDevGetSens', repmat(struct(), 1, num_sensors));

            % Call dll function
            status = calllib(DeviceLib.alias(), 'TMSiGetDeviceSensor', handle, ...
            ...
            channel_list_ptr = TMSiUtils.toMatlabStructure('TMSiDevGetSens', channel_list_ptr, num_sensors);
        ...
        function resetDeviceDataBuffer(handle)
            ...
            % Call dll function
            status = calllib(DeviceLib.alias(), 'TMSiResetDeviceDataBuffer', handle);
            if status ~= 0
        ...
        function [data, num_sets] = getDeviceRepairData(handle, buffer, buffer_len, repair_request)
            ...
            repair_request = TMSiUtils.toLibraryStructure('TMSiDevRepairReq', repair_request);

            % Call dll function
            status = calllib(DeviceLib.alias(), 'TMSiGetDeviceRepairData', handle, buffer, buffer_len, num_sets, repair_request);
            if status ~= 0
