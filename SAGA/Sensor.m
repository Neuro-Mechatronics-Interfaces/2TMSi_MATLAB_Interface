        ...
        function obj = Sensor(device, sensor_info) 
            ...
            if numel(sensor_info) > 1
                ...
                for ii = 1:numel(sensor_info)
                    obj(ii) = Sensor(device, sensor_info(ii));
                end
                return;
            end
...
%             obj.channels = SensorChannelDummy.empty;
            channels = {};
    ...
    methods(Access = private)
        function sensor_channel = createChannel(obj, channel_number, meta_data) 
            ...
            switch struct_id
                case hex2dec('0000')
                    sensor_channel = SensorChannelType0(obj, channel_number, meta_data);
                case hex2dec('FFFF')
                    sensor_channel = SensorChannelDummy(obj, channel_number, meta_data); 
                otherwise
