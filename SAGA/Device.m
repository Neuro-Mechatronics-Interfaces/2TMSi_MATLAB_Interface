classdef Device < HiddenHandle
    ...
    methods (Access = private)
        ...
        function updateDeviceConfig_(obj, perform_factory_reset, store_as_default, web_interface_control)
            ...
            device_config = struct( ...
                ...
                'SetConfiguredInterface', uint16(TMSiUtils.toInterfaceTypeNumber(obj.data_recorder.interface_type)), ...
                'SetTriggers', int16(obj.configuration.triggers), ...
                'SetRefMethod', int16(TMSiUtils.toReferenceMethodNumber(obj.configuration.reference_method)), ...
            ...
            DeviceLib.setDeviceConfig(obj.handle, device_config, channels);
            
            
            TMSiUtils.info(obj.name, 'sent device configuration')
    ...
    methods
        function obj = Device(lib, device_id, dr_interface_type)
            ...
            obj.lib = lib;
            obj.data_recorder = DataRecorderInfo();
            obj.docking_station = DockingStationInfo();
        ...
        function connect(obj, dr_interface_type)
            ...
            % Connect device
            obj.handle = DeviceLib.openDevice(obj.device_id, ...
                TMSiUtils.toInterfaceTypeNumber(dr_interface_type));
            
            obj.is_connected = true;
            TMSiUtils.info('MATLAB', 'opened connection to device')
        ...
        function disconnect(obj)
            ...
            DeviceLib.closeDevice(obj.handle);
            obj.is_connected = false;
            
            TMSiUtils.info(obj.name, 'closed connection to device')
        ...
        function start(obj, disable_avg_ref_calculation)
            ...
            if obj.prepared_sample_buffer_length == 0
                ...
                obj.prepared_sample_buffer = DeviceLib.createDataBuffer(obj.prepared_sample_buffer_length);
            end
            ...
            obj.missing_samples = [];
            DeviceLib.resetDeviceDataBuffer(obj.handle);
            
            % Set sampling request
            if obj.impedance_mode
                DeviceLib.setDeviceImpedance(obj.handle, struct('SetImpedanceMode', uint16(1)));
            else
                ...
                DeviceLib.setDeviceSampling(obj.handle, device_sample_request);
            end
            
            obj.is_sampling = true;
            
            % Inform user on start of sampling
            TMSiUtils.info(obj.name, 'started sampling from device')
            TMSiUtils.info(obj.name, ['    autoswitch=' num2str(obj.configuration.auto_reference_method)])
            TMSiUtils.info(obj.name, ['    repair_logging=' num2str(obj.configuration.repair_logging)])
            TMSiUtils.info(obj.name, ['    avr_ref_calc=' num2str(~disable_avg_ref_calculation)])
        ...
        function stop(obj)
            ...
            if obj.impedance_mode
                DeviceLib.setDeviceImpedance(obj.handle, struct('SetImpedanceMode', uint16(0)));
            else
                device_sample_request = struct( ...
            ...
            TMSiUtils.info(obj.name, 'stopped sampling from device')
        ...
        function [data, num_sets, data_type] = sample(obj)
            ...
            [raw_data, num_sets, data_type] = DeviceLib.getDeviceData(obj.handle, obj.prepared_sample_buffer, obj.prepared_sample_buffer_length);
        ...
        function [data_d, num_sets] = getMissingSamples(obj)
            ...
            TMSiUtils.info(obj.name, 'start retrieving missing samples')
            ...
            for i=1:2:numel(obj.missing_samples)
                ...
                for j=1:max_number_of_sample_sets:num_sample_sets
                    ...
                    [d, n] = DeviceLib.getDeviceRepairData(obj.handle, obj.prepared_sample_buffer, obj.prepared_sample_buffer_length, repair_request);
                ...
                TMSiUtils.info(obj.name, ['at sample ' num2str(num_repaired) ' of ' num2str(num_samples)]);
            end
            ...
            TMSiUtils.info(obj.name, 'transforming missing sample data')
            ...
            TMSiUtils.info(obj.name, 'done retrieving missing samples')
        end
        ...
        function resetDeviceConfig(obj)
            ...
            TMSiUtils.info(obj.name, 'reset configuration back to factory settings')
            obj.disconnect
            obj.connect
        end
        ...
        function getDeviceStatus(obj)
            ...
            device_report = DeviceLib.getDeviceStatus(obj.handle);
            ...
            obj.docking_station.interface_type = TMSiUtils.toInterfaceTypeString(device_report.DSInterface);
            obj.data_recorder.interface_type = TMSiUtils.toInterfaceTypeString(device_report.DRInterface);
            ...
            [device_report, battery_report_list, time_report, storage_report] = ...
                DeviceLib.getFullDeviceStatus(obj.handle, obj.num_batteries);
        ...
        function getDeviceConfig(obj)
            ...
            [rec_conf, channel_list] = DeviceLib.getDeviceConfig(obj.handle, obj.num_channels);
            ...
            obj.configuration.reference_method = TMSiUtils.toReferenceMethodString(int64(rec_conf.RefMethod));
            ...
            obj.channels = Channel(obj, channel_list);
%             obj.channels = Channel.empty;
            for i=1:numel(channel_list)
%                 channel_list(i).Number = i - 1;
%                 obj.channels(i) = Channel(obj, channel_list(i));
            ...
            sensor_list = DeviceLib.getDeviceSensors(obj.handle, obj.num_sensors);
            obj.sensors = Sensor(obj, sensor_list);
        ...
        function setDeviceConfig(obj, config)
            ...
            if isfield(config, 'Dividers')
                for i = 1:size(config.Dividers,1)
                    ...
                    obj.dividers(TMSiUtils.toChannelTypeNumber(config.Dividers{i,1})) = config.Dividers{i,2};
                end
            end
