classdef Poly5 < HiddenHandle
    ...
    %	close - Close file.
    %	STATIC read - Read Poly5 and store it as a Data object.
    ...
    %
    %   poly5 = Poly5('Example', device.sample_rate, device.getActiveChannels());
    %
    %   device.start();
    %
    %   for i=1:10
    ...
    methods
        function obj = Poly5(filepath, sample_rate, channels)
            ...
            if numel(filepath) > 1
                ...
                for ii = 1:numel(filepath)
                    obj(ii) = Poly5(filepath(ii), sample_rate(ii), channels{ii});
                    pause(1);
                end
                return;
            end
            ...
            for i=1:numel(obj.channels)
                if obj.channels(i).type == TMSiUtils.toChannelTypeNumber('digstat')
                    obj.digstat_channel = i;
                end
                
                if obj.channels(i).type == TMSiUtils.toChannelTypeNumber('counter')
                    obj.counter_channel = i;
                end
            end
            
            % Write the header and channel descriptions in the Poly5 file
            Poly5.writeHeader(obj.handle, obj, obj.num_samples_per_block);
            Poly5.writeChannelDescriptions(obj.handle, obj);
        ...
        function append(obj, samples)
            ...
            while size(obj.samples, 2) >= obj.num_samples_per_block
                ...
                Poly5.writeDataBlock(obj.handle, obj.num_samples_per_block, obj.num_samples / obj.num_samples_per_block, obj.samples(:, 1:obj.num_samples_per_block));
    ...
    methods(Static)
        function data = read(filepath)
            ...
            %   data = read(filepath)
            %
            %	Loads a Poly5 file and outputs the file as a Data
            %	object.
            %
            ...
            %	HEADER
            % ===========================================
            header = Poly5.readHeader(handle);
            
            % ===========================================
            %	SIGNAL DESCRIPTION
            % ===========================================
            channels = Poly5.readChannelDescriptions(handle, header.num_channels);
            ...
            for i=1:header.num_data_blocks
                ...
                d =  Poly5.readDataBlock(handle, numel(channels), i_end - i_start + 1);
            ...
            data = Data(header.name, header.sample_rate, channels, samples);
        end
        ...
        function write(filepath, data)
            ...
            %	HEADER
            % ===========================================
            Poly5.writeHeader(handle, data, num_samples_per_block);
            
            % ===========================================
            %	SIGNAL DESCRIPTION
            % ===========================================
            Poly5.writeChannelDescriptions(handle, data);
            ...
            for i=1:ceil(size(data.samples, 2) / num_samples_per_block)
                ...
                Poly5.writeDataBlock(handle, num_samples_per_block, i - 1,  data.samples(:, i_start:i_end));
            end
