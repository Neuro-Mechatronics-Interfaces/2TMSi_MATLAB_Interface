function mdl = load_ab_saga_poly5_and_train_classifier(SUBJ, YYYY, MM, DD, BLOCK, options)
...
[~, labels] = parse_instruction_triggers(...
...
X = interp1(movmean(X,options.DecimationFactor,1),(1:options.DecimationFactor:size(X,1))','previous');
...
classifier_filename = sprintf("%s/%s/%s/%s_%s_%d.poly5", options.InputRoot, SUBJ, TANK, TANK, options.ClassifierFileID, BLOCK);
