...
if exist('controller', 'var')~=0
    if ~iscell(controller)
        delete(controller);
    end
    clear controller;
end
controller = tcpserver(SERVER_ADDRESS, SERVER_PORT_CONTROLLER, ...
...
controller.UserData.udp.EnableBroadcast = true;
configureCallback(controller, "terminator", @(src, evt)server__CON_read_data_cb(src, evt));