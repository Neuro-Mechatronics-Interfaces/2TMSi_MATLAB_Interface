%TEST_SAGA_EMG_CONTACTILE Test script to check on polling synchronously from 2 TMSi-SAGA + converting SAGA polling into emulated control commands based on EMG RMS.
clc;
...
CONTACTILE_PORT = "COM8"; % REQUIRED to have Contactile hub on USB COM port!
...
all_ch = active_channels_2_sync_channels(ch, ...
    ...
    'NumContactileSensors',6);
    % 'NumContactileSensors', 1); % For testing
contactile_channels = [1:9, 16:24]; % First 3 on SEN0, First 3 on SEN1
% contactile_channels = 19:21;
...
sp = init_contactile_serialport_listener("COMPort",CONTACTILE_PORT);

SP_READBYTES_CONTACTILE = 195;
...
sp.read(sp.NumBytesAvailable, "uint8"); % "Flush" buffer
...
while isvalid(fig) && isvalid(muap_fig)
    ...
    try
        contactile_data = sp.read(max(SP_READBYTES_CONTACTILE,sp.NumBytesAvailable), "uint8");
        contactile_data = decode_contactile_frames(reshape(contactile_data((end-SP_READBYTES_CONTACTILE+1):end), 195, batch_samples_contactile)');
        % sp_vec = sp.UserData.SampleIndex:(sp.UserData.SampleIndex+batch_samples_contactile-1);
        % render_contactile_dataframe(sp.UserData.LineHandles, sp.UserData.Labels, contactile_data, sp_vec);
        % Increment sample index with wrap-around
        % sp.UserData.SampleIndex = mod(sp.UserData.SampleIndex-1+batch_samples_contactile, sp.UserData.SamplesPerTrace) + 1;
        
    catch
        contactile_data = zeros(30, batch_samples_contactile);
    end
    ...
    combined_data = [data; buttonData; teensyData; repelem(contactile_data(contactile_channels,:),1,n_replicates_contactile); tsData];
    if SAVE_DATA
...
% function render_contactile_dataframe(hLines, hLabels, samples, vec)
%     for ch = 1:30
%         sensorIdx = ceil(ch / 3);
%         if any(isnan(samples(ch,:)))
%             channelValue = nan(1,size(samples,2));
%             hLabels(sensorIdx).Visible = matlab.lang.OnOffSwitchState.off;
%         else
%             channelValue = samples(ch,:);
%             if hLabels(sensorIdx).Visible == matlab.lang.OnOffSwitchState.off
%                 hLabels(sensorIdx).Visible = matlab.lang.OnOffSwitchState.on;
%             end
%         end
%         hLines(ch).YData(vec) = channelValue + hLines(ch).UserData; % Adds the Y-Offset
%     end
% 
% end