...
timerObj.UserData = struct(...
    ...
    'B', struct('x', (1:config.Default.N_Spike_Channels)', ...
                'y', zeros(config.Default.N_Spike_Channels,1)), ...
    'Zprev', zeros(config.Default.N_Spike_Channels*2,1), ...
    'NTotal', 2*config.Default.N_Spike_Channels, ...
    'CurrentReportedPose', TMSiAccPose.Unknown, ...
    'CurrentAssignedPose', 0, ...
    'Data', struct( ...
        'Y', zeros(0,1), ...
        'X', zeros(0, config.Default.N_Spike_Channels*2), ...
        'Pose', zeros(0,1))); 
...
% timerObj.TimerFcn = @(src,~)bar_height_update_callback(src, hA, hB, hTxt, hTxt2, autoEnc, net, Label, hZ);
