...
| ** Pose Threshold** | `t` | Sets 10x the number of g's (fixed across accelerometer axes) for determining pose using differential accelerometery.  Example: `writeline(udpSock, "t.450", "192.168.88.100", 3036);` sets the threshold to be 4.5g for the Distal/Proximal, Medial/Lateral, and Superior/Inferior axes on the index-finger differential accelerometer pair. |
