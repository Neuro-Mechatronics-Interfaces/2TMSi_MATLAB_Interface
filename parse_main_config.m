    ...
    for ik = 1:numel(ch_type_opts)
        if isfield(config.SAGA.(tag).Channels, ch_type_opts(ik))
            ...
            if startsWith(upper(string(config.SAGA.(tag).Array.Type)),"TEXTILE") && strcmpi(chs,"UNI")
                config.SAGA.(tag).Channels.(chs) = config.SAGA.(tag).Channels.(chs)([17	16	15	14	13	9	5	1	22	21	20	19	18	10	6	2	27	26	25	24	23	11	7	3	32	31	30	29	28	12	8	4	33	34	35	36	37	53	57	61	38	39	40	41	42	54	58	61	43	44	45	46	47	55	59	63	48	49	50	51	52	56	60	64]);
            end
        end
    end
end
