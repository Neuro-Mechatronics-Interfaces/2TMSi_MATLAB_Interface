function [snips, idx, clus, nts, included_ch, isi_data, coeff, Y] = muap_templates(data, K, options)
...
arguments
    ...
    options.FilterCutoff (1,1) double = 10; % Cutoff frequency (Hz)
    options.Vector (1,:) double {mustBeInteger} = -10:10;
    options.MinPeakHeight (1,1) double = 15;
    options.MinPeakDistance (1,1) double {mustBePositive, mustBeInteger} = 10;
    ...
    options.PrincipalComponent (1,1) double {mustBePositive, mustBeInteger} = 1;
...
[b,a] = butter(3, options.FilterCutoff / (data.sample_rate/2), 'high');
uni_ch = find(startsWith({data.channels.alternative_name},'UNI'));
uni_d = filtfilt(b,a,data.samples(uni_ch,:)');

[coeff,score] = pca(uni_d);
[~,idx] = findpeaks(score(:,options.PrincipalComponent), ...
...
if options.PlotISI
    ...
    for ii = 1:K
        ...
        isi_data(iAscend(ii)).n = numel(deltas);
        isi_data(iAscend(ii)).cv = median(abs(deltas - median(deltas)))/median(deltas);
    ...
    isi_data = struct('n',cell(K,1),'cv',cell(K,1));
    for ii = 1:K
        deltas = diff(idx(clus==ii)./(data.sample_rate*1e-3));
...
if options.PlotTemporal
    ...
    line(ax, t_uni, score(:,options.PrincipalComponent), ...
