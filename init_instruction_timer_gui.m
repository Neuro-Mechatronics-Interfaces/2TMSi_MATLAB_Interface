...
fig.UserData.Timer = timer('Name','GesturesTimer-0',...
    'ExecutionMode','fixedRate',...
    'Period',0.100,'TimerFcn',@runGesturesList);
fig.UserData.Timer.UserData = struct(...
    'UDP',udpport("LocalPort", config.UDP.Socket.TimerGUI.Port, 'Timeout', 30),... 
    'Target',config.UDP.Socket.GesturesGUI, ...
    'CurrentRep',1,'CurrentGesture',1, ...
    'RestDuration',5,'GestureDuration',5,...
    'Reps',10,'Gestures',numel(options.GestureList),...
    'GestureList',1:numel(options.GestureList), ...
    'tCur', [], 'State', 0);
